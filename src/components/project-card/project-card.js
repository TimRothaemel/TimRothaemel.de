...
async function loadProjects() {
    try {
        ...
        // Load translations from the central lang files (fall back to defaults)
        try {
            const langResp = await fetch(`/lang/${currentLanguage}.json`);
            translations = langResp.ok ? await langResp.json() : {};
        } catch (err) {
            translations = {};
        }
        ...
        if (container) container.innerHTML = '<p>Fehler beim Laden der Projekte.</p>';
    }
}

// Seitentexte aktualisieren
function updatePageLanguage() {
    document.documentElement.lang = currentLanguage;
    const titleEl = document.getElementById('page-title');
    if (titleEl) titleEl.textContent = t('projects');
}
